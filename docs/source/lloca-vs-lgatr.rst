...
* `L-GATr-slim <https://arxiv.org/abs/2512.17011>`_, a Lorentz-equivariant transformer using scalar and vector representations
...
* **Reduced resource constraints**: For similar network size, we find that LLoCa networks typically require less GPU memory and FLOPs than comparable specialized-layer networks, e.g. L-GATr-slim.
  ...
  The overhead from the Frames-Net and frame-to-frame conversions in message passing increases inference time by a factor of 1x to 1.5x in our current implementation,
  roughly matching the FLOPs overhead and indicating that our implementation is close to optimal.
